...
bool SecurityManager::checkPasswordOnStartup()
{
    ...
    emit masterPasswordVerified(""); //Emit empty string when no password required
...
void SecurityManager::onTogglePasswordProtection(bool enabled)
{
    if (enabled) {
        ...
        if (infoResult != QMessageBox::Ok) {
            disconnect(m_requirePasswordAction, &QAction::toggled,
        ...
        // ALWAYS set master password when enabling protection
        setMasterPassword();

        // If password was actually set, enable protection
